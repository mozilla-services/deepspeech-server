...
impl ArgsParser {
	...
	pub fn from_cli() -> RuntimeConfig {
		let matches = clap::App::new("DeepSpeech Inference Server")
							  ...
							  .arg(clap::Arg::with_name("http_ip")
								   .short("h")
								   .long("http_ip")
								   .value_name("HTTP_IP")
							  ...
							  .arg(clap::Arg::with_name("model")
								   ...
								   .takes_value(true)
								   .required(true))
							  .arg(clap::Arg::with_name("alphabet")
								   .short("a")
								   .long("alphabet")
								   .value_name("ALPHABET")
...
#[test]
fn test_to_ip_addr() {
	assert_eq!(ArgsParser::to_ip_addr(Some("")), IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)));
	...
	assert_eq!(ArgsParser::to_ip_addr(Some("ffx3::1")), IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)));
